...
class Breadcrumb_Trail {
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'separator'       => ' \ ',
			'separator_class' => 'eac-breadcrumbs-separator eac-icon-svg',
			'home_icon'       => '',
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				// Build the list item.
				$breadcrumb .= sprintf( '<%1$s %2$s><span dir="ltr">%3$s</span></%1$s>', tag_escape( $this->args['item_tag'] ), $attributes, $item );
