...
class Site_Title_Widget extends Widget_Base {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				array(
					...
					'default'   => '#000000',
					'selectors' => array(
						'{{WRAPPER}} .elementor-heading-title, {{WRAPPER}} .eac-site-title a' => 'color: {{VALUE}};',
					),
				)
			);
	...
	protected function render() {
		...
		if ( 'custom' === $head_type_link && ! empty( $settings['site_title_link']['url'] ) ) {
			...
			$this->add_render_attribute( 'st_link_to', 'title', esc_html( $title ) );
			$this->add_render_attribute( 'st_link_to', 'aria-label', esc_html__( "Page d'accueil", 'eac-components' ) );
		...
		<div class='eac-site-title-wrapper elementor-widget-heading' itemprop='name'>
			<?php if ( $head_link_url ) { ?>
				<<?php echo esc_attr( $site_title_tag ); ?> class='eac-site-title elementor-heading-title'>
				<a <?php $this->print_render_attribute_string( 'st_link_to' ); ?>><?php echo esc_html( $title ); ?></a>
			...
			<?php } else { ?>
				<<?php echo esc_attr( $site_title_tag ); ?> class='eac-site-title elementor-heading-title'><?php echo esc_html( $title ); ?></<?php echo esc_attr( $site_title_tag ); ?>>
			<?php } ?>
		</div>
		<?php
	}
	...
	protected function content_template() {
		...
		<div class='eac-site-title-wrapper elementor-widget-heading' itemprop='name'>
			...
			<# } #>
			<{{{ siteTitleTag }}}  class='eac-site-title elementor-heading-title'>
