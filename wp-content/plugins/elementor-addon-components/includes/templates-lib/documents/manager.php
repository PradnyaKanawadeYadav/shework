...
final class Manager {
	...
	public function __construct() {
		...
		$this->current_theme = strtolower( esc_html( get_template() ) );
	...
	private function has_assigned_template( $type ) {
		...
		if ( $_tpl && 'inherit' !== $_tpl ) {
			if ( 'default' === $_tpl ) {
				...
				$template = get_page_by_path( $_tpl, OBJECT, Source_Local::CPT );
			}
		}
	...
	private function get_assigned_template( $template_type, $page_type ) {
		...
		if ( $template ) {
			return get_page_by_path( $template, OBJECT, Source_Local::CPT );
		}
		...
		if ( ! empty( $type ) && in_array( $type, array( SiteHeader::TYPE, SiteFooter::TYPE ), true ) ) {
			...
			if ( isset( $settings['show_on'] ) ) {
				switch ( $settings['show_on'] ) {
					case 'global':
						update_option( $key_prefix . 'global', $post_name, 'no' );
						break;
					case 'blog':
					case 'index':
						update_option( $key_prefix . 'blog', $post_name, 'no' );
						break;
					case 'front':
						update_option( $key_prefix . 'front', $post_name, 'no' );
						break;
					case 'search':
						update_option( $key_prefix . 'search', $post_name, 'no' );
						break;
					case 'err404':
						update_option( $key_prefix . 'err404', $post_name, 'no' );
						break;
					case 'wc_shop':
						update_option( $key_prefix . 'wc_shop', $post_name, 'no' );
						break;
					case 'privacy':
						update_option( $key_prefix . 'privacy', $post_name, 'no' );
						break;
					case 'singular':
						if ( ! empty( $settings['singular_pages'] ) ) {
							foreach ( $settings['singular_pages'] as $page_type ) {
								update_option( $key_prefix . 'singular_' . $page_type, $post_name, 'no' );
							}
						} else {
							update_option( $key_prefix . 'singular', $post_name, 'no' );
						}
						break;
					case 'archive':
						if ( ! empty( $settings['archive_pages'] ) ) {
							foreach ( $settings['archive_pages'] as $page_type ) {
								update_option( $key_prefix . 'archive_' . $page_type, $post_name, 'no' );
							}
						} else {
							update_option( $key_prefix . 'archive', $post_name, 'no' );
						}
						break;
					case 'custom':
						if ( ! empty( $settings['singular_pages'] ) ) {
	...
	public function enqueue_scripts() {
		wp_enqueue_style( 'eac-ehf', EAC_Plugin::instance()->get_style_url( 'includes/templates-lib/assets/css/eac-ehf' ), array( 'eac-frontend' ), '2.1.0' );
