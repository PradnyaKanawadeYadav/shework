 ...
 *
 * @param $element. Le contenu du menu de navigation
 * @since 2.1.0
 */
class widgetMegaMenu extends elementorModules.frontend.handlers.Base {
	...
	onInit() {
		...
		if (this.elements.mediaQuery.matches) {
			...
			this.mobileButtonsToggleOn();
		} else {
			this.mobileBindMouseEvents();
		}
	 ...
	 */
	mobileButtonsToggleOn() {
		this.mobileBindButtonsEvents();
		this.mobileUnbindMouseEvents();
	 ...
	 */
	mobileButtonsToggleOff() {
		this.mobileUnbindButtonsEvents();
		this.mobileBindMouseEvents();
	...
	mobileBindButtonsEvents() {
		...
		this.elements.$target_top_links.on('click', this.onClickTopLink.bind(this));
		this.elements.$target_sub_links.on('click', this.onClickSubLink.bind(this));
	}

	/**
	 * Suppression des événements sur les éléments concernés par le menu responsive
	 */
	mobileUnbindButtonsEvents() {
    ...
    /** Éviter le scroll du body avec les mobiles */
    mobileBindScrollEvent() {
		this.elements.$targetInstance.find('.mega-menu_nav-wrapper:not(.breakpoint) li.mega-menu_top-item.menu-item-has-children>a').on('touchstart touchmove touchend', (evt) => {
			evt.preventDefault();
			jQuery(evt.currentTarget).trigger('focus');
			jQuery(evt.currentTarget).css('outline', '0');
		});

        this.elements.$targetInstance.find('.mega-menu_nav-wrapper:not(.breakpoint) li.mega-menu_sub-item.menu-item-has-children>a').on('touchstart touchmove touchend', (evt) => {
			evt.preventDefault();
			jQuery(evt.currentTarget).trigger('focus');
			jQuery(evt.currentTarget).css('outline', '0');
		});
