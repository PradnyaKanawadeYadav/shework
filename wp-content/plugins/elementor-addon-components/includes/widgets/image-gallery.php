...
class Image_Galerie_Widget extends Widget_Base {
	...
	public function __construct( $data = array(), $args = null ) {
		...
		wp_register_script( 'eac-image-gallery', EAC_Plugin::instance()->get_script_url( 'assets/js/elementor/eac-image-gallery' ), array( 'jquery', 'elementor-frontend', 'swiper-bundle', 'isotope', 'eac-imagesloaded', 'fj-gallery' ), EAC_PLUGIN_VERSION, true );
		...
		wp_register_style( 'eac-image-gallery', EAC_Plugin::instance()->get_style_url( 'assets/css/image-gallery' ), array( 'eac-swiper', 'fj-gallery' ), EAC_PLUGIN_VERSION );
	}
	...
	public function get_script_depends(): array {
		return array( 'swiper-bundle', 'isotope', 'eac-imagesloaded', 'fj-gallery', 'eac-image-gallery' );
	}
	...
	protected function register_controls(): void {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				array(
					...
					'default'      => 'yes',
					'render_type'  => 'template',
					'prefix_class' => 'image-galerie__ratio-',
			...
			$this->add_responsive_control(
				...
				array(
					...
					'selectors' => array(
						...
						{{WRAPPER}} .swiper .image-galerie__content.overlay-out .image-galerie__overlay' => 'align-items: {{VALUE}};',
		...
		$this->start_controls_section(
			...
			$this->register_button_more_content_controls( array( 'control_condition' => array( 'ig_content_readmore' => 'yes' ) ) );
		...
		$this->start_controls_section(
			...
			$this->add_group_control(
				...
				array(
					'name'      => 'ig_container_style_border',
			...
			$this->add_control(
				...
				array(
					...
					'selectors'          => array(
						'{{WRAPPER}} .image-galerie__inner-wrapper' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					),
					'condition'          => array( 'ig_img_style' => 'style-0' ),
				)
			);
			...
			$this->add_control(
				...
				array(
					...
					'global'    => array( 'default' => Global_Colors::COLOR_SECONDARY ),
					'selectors' => array(
						'{{WRAPPER}} .ig-filters__wrapper .ig-filters__item.ig-active:after' => 'border-bottom: 3px solid {{VALUE}};',
						'{{WRAPPER}} .ig-filters__wrapper .ig-filters__item.ig-active a' => 'color: {{VALUE}};',
					),
					'condition' => array(
						'ig_layout_type!'           => array( 'justify', 'slider' ),
						'ig_content_filter_display' => 'yes',
			...
			// Trait du contenu du bouton read more
			$this->register_button_more_style_controls( array( 'control_condition' => array( 'ig_content_readmore' => 'yes' ) ) );
	...
	protected function render() {
		...
		if ( $has_swiper ) { ?>
			<div id="<?php echo esc_attr( $slider_id ); ?>" class='eac-image-galerie swiper'>
		<?php } else { ?>
	...
	protected function render_galerie() {
		...
		$this->add_render_attribute( 'inner_wrapper', 'class', 'image-galerie__inner-wrapper' );
		...
		foreach ( $settings['ig_image_list'] as $index => $item ) {
			...
			/** Le titre, la description sont éditables en ligne */
			...
			if ( $has_filters && ! empty( $item['ig_item_filter'] ) ) {
				...
				if ( 'justify' === $layout_type ) {
					...
					$this->add_render_attribute( 'gallery_item', 'data-swiper-slide-index', $index );
				} else {
			...
			if ( $item_link ) {
				$this->add_link_attributes( 'title_link', $item['ig_item_url'] );
				if ( $item['ig_item_url']['is_external'] ) {
					$this->add_render_attribute( 'title_link', 'rel', 'noopener noreferrer' );
				}
				if ( $has_global_link ) {
					$this->add_render_attribute( 'content_wrapper', 'class', 'image-galerie__content-wrapper global__card-link' );
					$this->add_render_attribute( 'title_link', 'class', 'eac-accessible-link card-link' );
				} else {
					$this->add_render_attribute( 'content_wrapper', 'class', 'image-galerie__content-wrapper' );
			...
			<article <?php $this->print_render_attribute_string( 'gallery_item' ); ?>>
				<div <?php $this->print_render_attribute_string( 'inner_wrapper' ); ?>>
					<div <?php $this->print_render_attribute_string( 'content_wrapper' ); ?>>
						...
						<?php if ( $has_title || $has_description || $has_button_readmore ) { ?>
							...
							if ( $item_link && $has_button_readmore ) {
								...
								$this->add_render_attribute( 'button_readmore', 'aria-label', esc_html( $settings['button_more_label'] ) . ' ' . esc_html( $item_title ) );
								?>
	...
	protected function get_settings_json() {
		...
		$module_settings = array(
			'data_id'                  => 'image_galerie_' . esc_attr( $this->get_id() ),
			'data_layout'              => $layout_type,
			'data_overlay'             => $overlay,
			'data_gutter'              => 'justify' === $layout_type && ! empty( $settings['ig_items_justify_margin']['size'] ) ? absint( $settings['ig_items_justify_margin']['size'] ) : 10,
			'data_rowheight'           => 'justify' === $layout_type && ! empty( $settings['ig_image_height']['size'] ) ? absint( $settings['ig_image_height']['size'] ) : 200,
