...
class Open_Streetmap_Widget extends Widget_Base {
	 ...
	 */
	private $config_layers = EAC_PLUGIN_URL . 'includes/config/osm/osmTiles.json';
	 ...
	 */
	private $config_icons = EAC_PLUGIN_URL . 'includes/config/osm/osmIcons.json';
	...
	public function __construct( $data = array(), $args = null ) {
		...
		wp_register_script( 'marker-cluster', EAC_PLUGIN_URL . 'assets/js/elementor/eac-osm-markercluster.min.js', array( 'leaflet' ), '1.5.3', true );
		wp_register_script( 'fullscreen', EAC_Plugin::instance()->get_script_url( 'assets/js/elementor/eac-osm-fullscreen' ), array( 'leaflet' ), '2.1.0', true );
		wp_register_script( 'eac-leaflet', EAC_Plugin::instance()->get_script_url( 'assets/js/elementor/eac-osm-leaflet' ), array( 'jquery', 'elementor-frontend', 'leaflet', 'marker-cluster', 'fullscreen' ), '1.8.8', true );
	...
	public function enqueue_editor_scripts() {
		wp_enqueue_script( 'eac-nominatim', EAC_Plugin::instance()->get_script_url( 'assets/js/elementor/eac-osm-search' ), array( 'jquery' ), '1.8.8', true );
	}
		...
		$this->start_controls_section(
			...
			$repeater->start_controls_tabs( 'osm_markers_tabs' );
				...
				$repeater->start_controls_tab(
					...
					array(
						'label' => '<i class="eicon-map-pin eac-osm-tabs" aria-hidden="true"></i>',
					)
				);
				...
				$repeater->start_controls_tab(
					...
					array(
						'label' => '<i class="eicon-notes eac-osm-tabs" aria-hidden="true"></i>',
					)
				);
		...
		$this->start_controls_section(
			...
			$this->add_responsive_control(
				...
				array(
					...
					'selectors'   => array( '{{WRAPPER}} .osm-map_wrapper-map' => 'min-block-size: {{SIZE}}{{UNIT}};' ),
	...
	protected function render_map() {
		...
		$this->add_render_attribute( 'osm_marker', 'data-lat', esc_html( $center_lat ) );
		$this->add_render_attribute( 'osm_marker', 'data-lng', esc_html( $center_lng ) );
		?>
		<div <?php $this->print_render_attribute_string( 'osm_wrapper' ); ?>>
			<!-- La div de la carte -->
			<div id="<?php echo esc_attr( $id ); ?>" class="osm-map_wrapper-map"></div>

			<!-- Le marqueur central -->
			<div <?php $this->print_render_attribute_string( 'osm_marker' ); ?>>
				<div class="osm-map_marker-title"><?php echo esc_html( $center_title ); ?></div>
				...
				if ( ! empty( $marker['osm_markers_tooltip_lat'] ) && ! empty( $marker['osm_markers_tooltip_lng'] ) ) {
					...
					$this->add_render_attribute(
						...
						array(
							...
							'data-lat'   => esc_html( $marker['osm_markers_tooltip_lat'] ),
							'data-lng'   => esc_html( $marker['osm_markers_tooltip_lng'] ),
					...
					<div <?php $this->print_render_attribute_string( $key ); ?>>
						<div class="osm-map_marker-title"><?php echo esc_html( $marker['osm_markers_tooltip_title'] ); ?></div>
	...
	protected function get_settings_json() {
		...
		$settings = array(
			...
			'data_zoom'          => $zoomauto && ! empty( $module_settings['osm_settings_zoom'] ) ? absint( $module_settings['osm_settings_zoom'] ) : 12,
			...
			'data_import_url'    => esc_url( $file_import ),
