...
class Off_Canvas_Widget extends Widget_Base {
	...
	protected function register_controls(): void {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				array(
					...
					'prefix_class' => 'oc-offcanvas__canvas-',
				)
			);
				...
				$this->start_controls_tab(
					...
					$this->add_responsive_control(
						...
						array(
							...
							'selectors' => array(
								'{{WRAPPER}} .oc-offcanvas__wrapper-btn, {{WRAPPER}} .oc-offcanvas__wrapper-text' => 'margin-inline: {{VALUE}};',
							),
					...
					$this->add_control(
						...
						array(
							...
							'condition'    => array(
								...
								'oc_content_position!' => array( 'stretch', 'top', 'bottom' ),
							),
						)
					);
					...
					$this->add_control(
						...
						array(
							...
							'label_block' => true,
							'selectors'   => array(
								'{{WRAPPER}} .oc-offcanvas__wrapper-btn.sticky-button-left' => 'inset-block-start: {{SIZE}}%; transform: rotate(-90deg) translateX(-{{SIZE}}%);',
								'{{WRAPPER}} .oc-offcanvas__wrapper-btn.sticky-button-right' => 'inset-block-start: {{SIZE}}%; transform: rotate(90deg) translateX({{SIZE}}%);',
							),
							'condition'   => array( 'oc_icon_sticky' => 'yes' ),
						)
					);
					...
					$this->add_control(
						...
						array(
							...
							'selectors'          => array(
								'{{WRAPPER}} .button-icon' => 'margin-block: 0; margin-inline: {{LEFT}}px {{RIGHT}}px;',
							),
							'condition'          => array(
								'oc_trigger_type'   => 'button',
								'oc_icon_activated' => 'yes',
		...
		$this->start_controls_section(
			...
			$this->add_responsive_control(
				...
				array(
					...
					'label_block' => true,
					'selectors'   => array(
						'{{WRAPPER}}.oc-offcanvas__canvas-left .oc-offcanvas__wrapper-canvas,
						{{WRAPPER}}.oc-offcanvas__canvas-right .oc-offcanvas__wrapper-canvas' => 'inline-size: {{SIZE}}{{UNIT}};',
					),
					'condition'    => array( 'oc_content_position' => array( 'left', 'right' ) ),
				)
			);
			...
			$this->add_control(
				...
				array(
					...
					'default'   => 'yes',
					'toggle'    => false,
					'condition'    => array( 'oc_content_position' => array( 'stretch', 'top', 'bottom' ) ),
				)
			);
			...
			$this->add_responsive_control(
				...
				array(
					...
					'label_block' => true,
					'selectors'   => array(
						'{{WRAPPER}}.oc-offcanvas__canvas-stretch .oc-offcanvas__wrapper-canvas,
						{{WRAPPER}}.oc-offcanvas__canvas-bottom .oc-offcanvas__wrapper-canvas,
						{{WRAPPER}}.oc-offcanvas__canvas-top .oc-offcanvas__wrapper-canvas' => 'block-size: {{SIZE}}{{UNIT}};',
					),
					'condition'    => array(
						'oc_content_position' => array( 'stretch', 'top', 'bottom' ),
						'oc_content_height_auto' => 'no',
					),
				)
			);
		...
		$this->start_controls_section(
			...
			$this->add_group_control(
				...
				array(
					...
					'selector' => '{{WRAPPER}} .oc-offcanvas__canvas-header',
				)
			);
		...
		$this->start_controls_section(
			...
			$this->start_controls_tabs( 'oc_controls_tabs' );
				...
				$this->start_controls_tab(
					...
					$this->add_control(
						...
						array(
							...
							'selectors' => array(
								...
								'{{WRAPPER}} .oc-offcanvas__wrapper-btn:hover svg, {{WRAPPER}} .oc-offcanvas__wrapper-btn:focus svg' => 'fill: {{VALUE}};',
							),
						)
					);
					...
					$this->add_control(
						...
						array(
							...
							'selectors' => array(
								'{{WRAPPER}} .oc-offcanvas__wrapper-btn:hover, {{WRAPPER}} .oc-offcanvas__wrapper-btn:focus' => 'border-color: {{VALUE}};',
							),
						)
					);
	...
	protected function render_offcanvas() {
		...
		if ( 'button' === $trigger ) {
			...
			$this->add_render_attribute( 'trigger', 'class', array( 'oc-offcanvas__wrapper-trigger oc-offcanvas__wrapper-btn', $sticky_button ) );
			...
			$this->add_render_attribute( 'trigger', 'aria-label', esc_html__( 'Ouvrir la barre latérale', 'eac-components' ) . ' ' . esc_html( $settings['oc_display_text_button'] ) );
			...
			$this->add_inline_editing_attributes( 'oc_display_text', 'none' );
		}

		/** Le wrapper du déclencheur bouton ou texte */
		$this->add_render_attribute( 'oc_wrapper', 'class', 'oc-offcanvas__wrapper' );
		$this->add_render_attribute( 'oc_wrapper', 'id', esc_attr( $id ) );
		...
		$label = 'button' === $trigger ? esc_html( $settings['oc_display_text_button'] ) : esc_html( $settings['oc_display_text'] );
		...
		<div <?php $this->print_render_attribute_string( 'oc_wrapper' ); ?>>
			<?php if ( 'button' === $trigger ) { ?>
				<button <?php $this->print_render_attribute_string( 'trigger' ); ?>>
					<?php if ( $icon_button && 'before' === $settings['oc_position_icon_button'] ) { ?>
						<span class='button-icon eac-icon-svg'>
						<?php Icons_Manager::render_icon( $settings['oc_display_icon_button'], array( 'aria-hidden' => 'true' ) ); ?>
						</span>
					<?php } ?>
						<span <?php $this->print_render_attribute_string( 'oc_display_text_button' ); ?>><?php echo esc_html( $settings['oc_display_text_button'] ); ?></span>
					<?php if ( $icon_button && 'after' === $settings['oc_position_icon_button'] ) { ?>
						<span class='button-icon eac-icon-svg'>
						<?php Icons_Manager::render_icon( $settings['oc_display_icon_button'], array( 'aria-hidden' => 'true' ) ); ?>
						</span>
					<?php } ?>
			...
			<?php } elseif ( 'text' === $trigger ) { ?>
				<div <?php $this->print_render_attribute_string( 'trigger' ); ?>>
					<a <?php $this->print_render_attribute_string( 'a_fancybox' ); ?>>
						<span <?php $this->print_render_attribute_string( 'oc_display_text' ); ?>><?php echo esc_html( $settings['oc_display_text'] ); ?></span>
					</a>
				</div>
			<?php } ?>
		</div>
				...
				<div id='oc-offcanvas__canvas-content' class='oc-offcanvas__canvas-content'>
					...
					<?php } elseif ( 'tmpl_cont' === $content ) {
						...
						$args = array(
							...
							'depth'           => absint( $settings['oc_content_menu_level'] ),
						);
						// Affiche le menu
						wp_nav_menu( $args );
					} else {
		...
		<?php
		echo ob_get_clean(); // phpcs:disable WordPress.Security.EscapeOutput
	}
	...
	protected function get_settings_json() {
		...
		$settings = array(
			...
			'data_canvas_id' => 'offcanvas_' . esc_attr( $this->get_id() ),
