...
class Acf_Relationship_Widget extends Widget_Base {
	...
	public function __construct( $data = array(), $args = null ) {
		...
		if ( ! class_exists( Eac_Acf_Lib::class ) ) {
			require_once EAC_ACF_INCLUDES . 'dynamic-tags/eac-acf-lib.php';
		}
		...
		$this->start_controls_section(
			...
			$this->add_responsive_control(
				...
				array(
					...
					'selectors'  => array( ' {{WRAPPER}} .swiper' => 'padding-block: {{SIZE}}{{UNIT}}; padding-inline: {{SIZE}}{{UNIT}};' ),
					'condition'  => array( 'acf_relation_layout_type' => 'slider' ),
				)
			);
			...
			$this->add_responsive_control(
				...
				array(
					...
					'selectors'  => array(
						'{{WRAPPER}} .acf-relation_container:not(.swiper-wrapper)' => 'gap: {{SIZE}}{{UNIT}}; padding-block: calc({{SIZE}}{{UNIT}} / 2); padding-inline: calc({{SIZE}}{{UNIT}} / 2);',
					),
					'condition'  => array( 'acf_relation_layout_type!' => 'slider' ),
				)
			);
	...
	protected function render_relationship_content( $items = array() ) {
		...
		foreach ( $items as $item ) {
			...
			if ( $has_global_link && $permalink ) {
				$this->add_render_attribute( 'content_wrapper', 'class', 'acf-relation_content-wrapper global__card-link' );
			} else {
				$this->add_render_attribute( 'content_wrapper', 'class', 'acf-relation_content-wrapper' );
			}
			...
			<article id="<?php echo esc_attr( $item['id'] ); ?>" class="<?php echo esc_attr( $item['class'] ); ?>">
				<div class='acf-relation_inner-wrapper'>
					<div <?php $this->print_render_attribute_string( 'content_wrapper' ); ?>>
						...
						<div class='acf-relation_content'>
							...
							if ( $has_button && $permalink ) {
								...
								$this->add_render_attribute( 'button_readmore', 'aria-label', esc_html( $label ) . ' ' . esc_html( $item['post_title'] ) );
					...
					</div> <!-- Fin div acf-relation_content-wrapper -->
