...
class Modal_Box_Widget extends Widget_Base {
	...
	public function __construct( $data = array(), $args = null ) {
		...
		EAC_Plugin::instance()->register_script( 'eac-modalbox', 'assets/js/elementor/eac-modal-box', array( 'jquery', 'elementor-frontend' ), EAC_PLUGIN_VERSION,
			array(
				'strategy' => 'defer',
				'in_footer' => true,
			)
	...
	protected function register_controls(): void {
		...
		$this->start_controls_section(
			...
			$this->add_responsive_control(
				...
				array(
					...
					'selectors'      => array(
						...
						.modalbox-visible-{{ID}} .fancybox-content' => 'max-inline-size: {{SIZE}}{{UNIT}}; inline-size: 100%;',
					),
				)
			);
			...
			$this->add_responsive_control(
				...
				array(
					...
					'label_block' => true,
					'selectors'   => array( '.modalbox-visible-{{ID}} .fancybox-content' => 'max-block-size: {{SIZE}}px; block-size: 100%;' ),
					'condition'   => array( 'mb_type_content' => array( 'links', 'html' ) ),
				)
			);
			...
			$this->add_group_control(
				...
				array(
					'name'      => 'mb_modal_box_border',
					'selector'  => '.modalbox-visible-{{ID}} .fancybox-content, #modalbox-hidden-{{ID}}.fancybox-content',
					'separator' => 'before',
				)
			);
			...
			$this->add_control(
				...
				array(
					...
					'selectors' => array(
						...
						.modalbox-visible-{{ID}} .fancybox-content button.fancybox-close-small' => 'color: {{VALUE}}; opacity: 1;',
					),
				)
			);
		...
		/** La caption de la boîte modale 'links et html' ne peut pas être stylisé */
		$this->start_controls_section(
			'mb_header_style',
			array(
			...
			$this->add_control(
				...
				array(
					...
					'selectors'  => array(
						'{{WRAPPER}} .mb-modalbox__hidden-content-title' => 'padding-block: {{TOP}}{{UNIT}} {{BOTTOM}}{{UNIT}}; padding-inline: {{LEFT}}{{UNIT}} {{RIGHT}}{{UNIT}};',
					),
				)
			);
		...
		$this->start_controls_tabs( 'mb_controls_tabs' );
				...
				$this->start_controls_tab(
					...
					$this->add_control(
						...
						array(
							...
							'global'    => array( 'default' => Global_Colors::COLOR_PRIMARY ),
							'selectors' => array(
								'{{WRAPPER}} .mb-modalbox__wrapper-btn' => 'color: {{VALUE}};',
							),
						)
					);
				...
				$this->start_controls_tab(
					...
					$this->add_control(
						...
						array(
							...
							'global'    => array( 'default' => Global_Colors::COLOR_PRIMARY ),
							'selectors' => array(
								'{{WRAPPER}} .mb-modalbox__wrapper-btn:hover, {{WRAPPER}} .mb-modalbox__wrapper-btn:focus' => 'color: {{VALUE}};',
								'{{WRAPPER}} .mb-modalbox__wrapper-btn:hover svg, {{WRAPPER}} .mb-modalbox__wrapper-btn:focus svg' => 'fill: {{VALUE}};',
							),
						)
					);
					...
					$this->add_control(
						...
						array(
							...
							'global'    => array( 'default' => Global_Colors::COLOR_PRIMARY ),
							'selectors' => array(
								'{{WRAPPER}} .mb-modalbox__wrapper-btn:hover, {{WRAPPER}} .mb-modalbox__wrapper-btn:focus' => 'background-color: {{VALUE}};',
							),
						)
					);
					...
					$this->add_control(
						...
						array(
							...
							'selectors' => array(
								'{{WRAPPER}} .mb-modalbox__wrapper-btn:hover, {{WRAPPER}} .mb-modalbox__wrapper-btn:focus' => 'border-color: {{VALUE}};',
							),
						)
					);
				...
				$this->start_controls_tab(
					...
					$this->add_control(
						...
						array(
							...
							'selectors'  => array(
								'{{WRAPPER}} .mb-modalbox__wrapper' => 'padding-block: {{TOP}}{{UNIT}} {{BOTTOM}}{{UNIT}}; padding-inline: {{LEFT}}{{UNIT}} {{RIGHT}}{{UNIT}};',
							),
						)
					);
				...
				$this->start_controls_tab(
					...
					$this->add_control(
						...
						array(
							...
							'selectors' => array(
								'{{WRAPPER}} .mb-modalbox__wrapper-img:hover, {{WRAPPER}} .mb-modalbox__wrapper-img:focus' => 'opacity: {{SIZE}};',
							),
						)
					);
					...
					$this->add_group_control(
						...
						array(
							...
							'selector' => '{{WRAPPER}} .mb-modalbox__wrapper-img:hover, {{WRAPPER}} .mb-modalbox__wrapper-img:focus',
						)
					);
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				array(
					...
					'selectors'  => array( '{{WRAPPER}} .mb-modalbox__wrapper figure figcaption' => 'padding-block-start: {{SIZE}}{{UNIT}};' ),
				)
			);
	...
	protected function render_modal() {
		...
		$icon_button = false;
		$type_inline = false;
		$header      = 'yes' === $settings['mb_enable_header'] && ! empty( $settings['mb_texte_header'] ) ? esc_html( $settings['mb_texte_header'] ) : '';
		$slideclass  = '';
		...
		if ( 'button' === $trigger ) {
			...
			$label = $settings['mb_display_text_button'];
			$this->add_render_attribute( 'trigger', 'type', 'button' );
			$this->add_render_attribute( 'trigger', 'id', 'mb_labelby_' . esc_attr( $id ) );
			$this->add_render_attribute( 'trigger', 'class', 'mb-modalbox__wrapper-trigger mb-modalbox__wrapper-btn' );
			...
			$this->add_render_attribute( 'trigger', 'alt', esc_html( $image_alt ) );
			...
			if ( $has_caption ) {
				if ( 'attachment' === $settings['mb_caption_source'] && ! empty( $settings['mb_display_image']['id'] ) ) {
					...
					$label = ! empty( $settings['mb_caption_texte'] ) ? $settings['mb_caption_texte'] : '';
				}
			}
		} elseif ( 'text' === $trigger ) {
			$label = $settings['mb_display_texte'];
			$this->add_render_attribute( 'trigger', 'id', 'mb_labelby_' . esc_attr( $id ) );
			$this->add_render_attribute( 'trigger', 'class', 'mb-modalbox__wrapper-trigger mb-modalbox__wrapper-text' );
			$this->add_render_attribute( 'trigger', 'title', esc_html( $settings['mb_display_texte'] ) );
		}
		...
		$this->add_render_attribute( 'mb_wrapper', 'id', esc_attr( $id ) );
		$this->add_render_attribute( 'mb_wrapper', 'data-settings', $this->get_settings_json() );
		?>
		<div <?php $this->print_render_attribute_string( 'mb_wrapper' ); ?>>
				...
				<a <?php $this->print_render_attribute_string( 'a_fancybox' ); ?>>
					<button <?php $this->print_render_attribute_string( 'trigger' ); ?>>
						...
						if ( $icon_button && 'before' === $settings['mb_position_icon_button'] ) { ?>
							...
							<?php Icons_Manager::render_icon( $settings['mb_display_icon_button'], array( 'aria-hidden' => 'true' ) ); ?>
							</span>
						<?php }
			...
			<?php } elseif ( 'text' === $trigger ) { ?>
				<a <?php $this->print_render_attribute_string( 'a_fancybox' ); ?>>
					<div <?php $this->print_render_attribute_string( 'trigger' ); ?>><?php echo esc_html( $label ); ?></div>
				</a>
				<?php
			} else {
			...
			if ( $type_inline ) {
				ob_start();
				?>
				<div id="modalbox-hidden-<?php echo esc_attr( $id ); ?>" class="mb-modalbox__hidden-content-wrapper elementor-<?php echo esc_attr( $main_id ); ?>" role='dialog' aria-labelledby="modal-<?php echo esc_attr( $id ); ?>" aria-modal='true'>
	...
	protected function get_settings_json() {
		...
		$module_settings = array(
			...
			'data_duration'   => ! empty( $settings['mb_modal_box_duration'] ) ? absint( $settings['mb_modal_box_duration'] ) : 600,
			'data_modal'      => true,
			'data_slideclass' => 'modalbox-visible-' . esc_attr( $this->get_id() ),
		);
