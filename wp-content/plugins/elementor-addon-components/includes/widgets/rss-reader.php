...
class Lecteur_Rss_Widget extends Widget_Base {
	 ...
	 *
	 * In the RSS widget, this implementation is used to pass a pre-rendered version of the icon to the front end,
	...
	public function get_frontend_settings(): array {
		...
		if ( Plugin::$instance->experiments->is_feature_active( 'e_font_icon_svg' ) && ! empty( $frontend_settings['button_more_picto']['value'] ) ) {
			$frontend_settings['button_more_picto']['rendered_tag'] = Icons_Manager::render_font_icon( $frontend_settings['button_more_picto'] );
		}
	...
	protected function register_controls(): void {
		...
		foreach ( $active_breakpoints as $breakpoint_name => $breakpoint_instance ) {
			...
			$this->add_responsive_control(
				...
				array(
					...
					'selectors'      => array( '{{WRAPPER}} .rss-galerie__item-image img' => 'block-size: {{SIZE}}{{UNIT}};' ),
			...
			$this->add_responsive_control(
				...
				array(
					...
					'selectors'   => array( '{{WRAPPER}} .rss-galerie__item-image img' => 'object-position: 50% {{SIZE}}%;' ),
					'render_type' => 'ui',
					'condition'   => array( 'rss_item_image_height_auto!' => 'yes' ),
				)
			);
			...
			$this->add_responsive_control(
				...
				array(
					...
					'toggle'    => false,
					'selectors' => array(
						'{{WRAPPER}} .rss-galerie__content-inner' => 'align-items: {{VALUE}}; text-align: {{VALUE}};',
						'{{WRAPPER}} .rss-galerie__content-inner .buttons-wrapper' => 'justify-content: {{VALUE}};',
					),
				)
			);
		...
		$this->start_controls_section(
			...
			$this->register_button_more_content_controls( array( 'control_condition' => array( 'rss_item_readmore' => 'yes' ) ) );
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				array(
					...
					'size_units' => array( 'px', '%' ),
					'selectors'  => array(
						'{{WRAPPER}} .eac-rss-galerie' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					),
				)
			);
			...
			$this->add_responsive_control(
				...
				array(
					...
					'selectors'  => array(
						'{{WRAPPER}} .rss-galerie' => 'gap: {{SIZE}}{{UNIT}}; padding-block: calc({{SIZE}}{{UNIT}} / 2); padding-inline: calc({{SIZE}}{{UNIT}} / 2);',
						'{{WRAPPER}} .rss-item__header' => 'margin-inline: calc({{SIZE}}{{UNIT}} / 2);',
					),
				)
			);
			...
			$this->add_control(
				...
				array(
					...
					'size_units' => array( 'px', '%' ),
					'selectors'  => array(
						'{{WRAPPER}} .rss-galerie__item' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					),
					'condition'  => array( 'rss_wrapper_style' => 'style-0' ),
				)
			);
			...
			$this->register_button_more_style_controls( array( 'control_condition' => array( 'rss_item_readmore' => 'yes' ) ) );
	...
	protected function render() {
		...
		<div class='eac-rss-galerie'>
			<input type='hidden' id='rss_nonce' name='rss_nonce' value="<?php echo wp_create_nonce( 'eac_rss_feed_' . esc_attr( $this->get_id() ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" />
			<?php $this->render_galerie(); ?>
			<div <?php $this->print_render_attribute_string( 'rss_galerie' ); ?>></div>
		</div>
		<?php
	}
	...
	protected function render_galerie() {
		...
		<div class='rss-select__item-list'>
			...
			<div class='eac__button'>
				<button id='rss__read-button' class='eac__read-button' type='button' aria-expanded='false' aria-controls='rss-galerie' aria-label="<?php echo esc_html( $settings['rss_item_button_label'] ) . ' ' . esc_html__( 'Afficher le contenu du flux sélectionné', 'eac-components' ); ?>">
					<?php echo esc_html( $settings['rss_item_button_label'] ); ?>
				</button>
			</div>
	...
	protected function get_settings_json() {
		...
		$module_settings = array(
			...
			'data_nombre'         => ! empty( $settings['rss_item_nombre'] ) ? absint( $settings['rss_item_nombre'] ) : 20,
