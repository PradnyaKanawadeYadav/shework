...
class Advanced_Image_Gallery_Widget extends Widget_Base {
	...
	public function __construct( $data = array(), $args = null ) {
		...
		wp_register_script( 'eac-advanced-gallery', EAC_Plugin::instance()->get_script_url( 'assets/js/elementor/eac-advanced-gallery' ), array( 'jquery', 'elementor-frontend', 'swiper-bundle', 'isotope', 'eac-imagesloaded', 'fj-gallery' ), EAC_PLUGIN_VERSION, true );
		...
		wp_register_style( 'eac-advanced-gallery', EAC_Plugin::instance()->get_style_url( 'assets/css/advanced-gallery' ), array( 'eac-frontend', 'eac-swiper', 'fj-gallery' ), EAC_PLUGIN_VERSION );
	}
	...
	public function get_script_depends(): array {
		return array( 'swiper-bundle', 'isotope', 'eac-imagesloaded', 'fj-gallery', 'eac-advanced-gallery' );
	}
		...
		$this->start_controls_section(
			...
			$this->add_responsive_control(
				...
				array(
					...
					'type'      => Controls_Manager::DIMENSIONS,
					'selectors' => array(
						'{{WRAPPER}} .advanced-gallery__overlay' => 'padding-block: {{TOP}}{{UNIT}} {{BOTTOM}}{{UNIT}}; padding-inline: {{RIGHT}}{{UNIT}} {{LEFT}}{{UNIT}};',
					),
					'condition' => array( 'ag_overlay_inout!' => 'overlay-out' ),
				)
			);
	...
	protected function render_gallery() {
		...
		/** La classe pour affecter les styles */
		$this->add_render_attribute( 'inner_wrapper', 'class', 'advanced-gallery__inner-wrapper' );

		if ( 'justify' === $layout_type ) {
			$overlay = 'overlay-in';
		} elseif ( ! isset( $settings['ag_overlay_inout'] ) ) {
			$overlay = '';
		} else {
		...
		foreach ( $settings['advanced_gallery'] as $image ) {
			...
			if ( $has_filters && ! empty( $attachment['media_cat'] ) ) {
				...
				$this->add_render_attribute( 'gallery_wrapper', 'class', 'advanced-gallery__item ' . implode( ' ', $sanized ) );
			} else {
				if ( 'justify' === $layout_type ) {
			...
			if ( $has_button_readmore && $media_url ) {
				...
				$this->add_render_attribute( 'button_readmore', 'aria-label', esc_html( $settings['button_more_label'] ) . ' ' . esc_html( $attachment['title'] ) );
			...
			if ( $has_global_link && $media_url ) {
				$this->add_render_attribute( 'content_wrapper', 'class', 'advanced-gallery__content-wrapper global__card-link' );
			} else {
				$this->add_render_attribute( 'content_wrapper', 'class', 'advanced-gallery__content-wrapper' );
			}
	...
	protected function get_settings_json() {
		...
		$module_settings = array(
			'data_id'                  => 'advanced_gallery_' . $this->get_id(),
