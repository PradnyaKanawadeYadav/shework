...
class Author_Infobox_Widget extends Widget_Base {
	...
	public function __construct( $data = array(), $args = null ) {
		...
		wp_register_style( 'eac-author-infobox', EAC_Plugin::instance()->get_style_url( 'assets/css/author-infobox' ), array( 'eac-frontend' ), '1.9.1' );
	}
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				array(
					...
					'ai'          => array( 'active' => false ),
					'label_block' => true,
					'default'     => esc_html__( 'Voir les archives', 'eac-components' ),
				)
			);
	...
	protected function render_infobox() {
		...
		if ( $has_role ) {
			...
			if ( ! empty( $user_info->roles ) && is_array( $user_info->roles ) ) {
				$title = implode( ', ', array_map( 'esc_html', $user_info->roles ) );
		...
		<div <?php $this->print_render_attribute_string( 'content_wrapper' ); ?>>
			<?php if ( ! empty( $avatar_url ) ) : ?>
				<div class='author-infobox_image'>
					<img class='avatar photo' src="<?php echo esc_url( $avatar_url ); ?>" alt='Author avatar photo' loading='lazy' width='150' height='150' />
				</div>
			<?php endif; ?>
				...
				<div class='author-infobox_info-content'>
					<?php if ( ! empty( $name ) ) : ?>
						<div class='author-infobox_name'>
							<?php echo $name; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
						</div>
					<?php endif; ?>
					<?php if ( ! empty( $role ) ) : ?>
					...
					<?php endif; ?>
					<?php if ( $has_button ) :
						$label = ! empty( $settings['aib_relation_more_label'] ) ? esc_html( $settings['aib_relation_more_label'] ) : esc_html__( 'Voir les archives', 'eac-components' );
						$this->add_render_attribute( 'button_readmore', 'class', 'eac-accessible-link button-readmore' );
						$this->add_render_attribute( 'button_readmore', 'role', 'button' );
						$this->add_render_attribute( 'button_readmore', 'aria-label', $label . ' ' . esc_html( $author_nicename ) );
