...
class Eac_Injection_KenBurns_Slideshow {
	...
	public function render_images( $element ) {
		...
		if ( isset( $settings['kb_element_active'] ) && 'yes' === $settings['kb_element_active'] ) {
			...
			?>
			<script type="text/javascript" kb-js-id="<?php echo esc_attr( $element->get_id() ); ?>">
					...
					if ( ! empty( $url ) ) {
						...
						<script type="text/javascript" kb-js-id="<?php echo esc_attr( $element->get_id() ); ?>">
							jQuery(document).ready(function() {
								...
								.append('<a class="slide" role="button" aria-disabled="true" tabindex="-1"><span class="<?php echo esc_attr( $animation ); ?>" style="background-image:url(\'<?php echo esc_url( $url ); ?>\'); background-position:<?php echo esc_attr( $position ); ?>; background-size:cover; background-repeat:no-repeat;"></span></a>');
							});
						</script>
						<?php
					}
