			...
			foreach ( array_reverse( $images_list ) as $index => $item ) {
				if ( ! empty( $item['bgi_image']['url'] ) ) {
					...
					if ( 'initial' === $item['bgi_size'] ) {
						if ( ! empty( $item['bgi_size_width_calc'] ) && 'calc' === substr( $item['bgi_size_width_calc'], 0, 4 ) ) {
							$size = $item['bgi_size_width_calc'];
						} else {
							$size = absint( $item['bgi_size_width']['size'] ) . $item['bgi_size_width']['unit'];
						}
					}
					$bg_size[]   = $size;
					$bg_repeat[] = $item['bgi_repeat'];
				}
			}
			if ( ! empty( $bg_url ) ) { ?>
			...
			_.each (images_list, function(item, index) {
				if (item.bgi_image.url && item.bgi_image.id) {
					...
					if (image_url) {
						...
						bg_size.push(size);
						bg_repeat.push(item.bgi_repeat);
					}
				}
			});
			if(! _.isEmpty(bg_url)) {
				var disp_url = bg_url.join(',');
				var disp_pos = bg_position.join(',');
				var disp_size = bg_size.join(',');
				var disp_repeat = bg_repeat.join(',');
				#>
				<div class="eac-background__images-wrapper" data-elem-id="{{ view.getID() }}" style="background-image:{{ disp_url }};background-position:{{ disp_pos }};background-repeat:{{ disp_repeat }};background-size:{{ disp_size }};background-attachment:scroll;"></div>
				<#
			}
		}
		#>
