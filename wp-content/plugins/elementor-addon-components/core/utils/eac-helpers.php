...
class Eac_Helpers_Util {
	...
	public static function wp_get_attachment_data( $attachment_id, $attachment_size, $filter = '', $the_id = -1, $count_element = 0 ) {
		...
		if ( in_array( $filter, array( 'selling', 'featured', 'category', 'product', 'recent' ), true ) && -1 !== $the_id ) {
			...
			if ( $post_parent ) {
				...
				$description = Eac_Tools_Util::get_post_excerpt( $parent_id, 100 );
			}
		}
	 ...
	 * @param { $term_filters}      Array: Un tableau de slug des étiquettes
	 *
	 * @return les filtres des catégories formatées en HTML
	 */
	public static function get_taxo_tag_filters( $taxonomy_filters, $term_filters, $id, $cat_parent = false ) {
		$html         = '';
		$unique_terms = array();
		...
		if ( ! is_wp_error( $terms ) && count( $terms ) > 0 ) {
			foreach ( $terms as $term ) {
				foreach ( $taxonomy_filters as $taxo ) {
					$prefix = 'post_tag' === $taxo ? 'tag-' : $taxo . '-';
					/**
					 * Catégorie parente
